...
const validateInvoice = (invoice: Partial<Invoice>): ValidationError[] => {
  ...
  invoice.items?.forEach((item, index) => {
    ...
    if (typeof item.unitPrice !== 'number' || item.unitPrice < 0) {
      errors.push({ field: `items[${index}].unitPrice`, message: 'Item unit price must be greater than or equal to 0' });
    }
  });
...
const getInvoices = async (page: number = 1, limit: number = 10): Promise<InvoiceResponse> => {
  try {
...
const createInvoice = async (invoice: Omit<Invoice, "_id" | "createdAt" | "updatedAt">) => {
  const validationErrors = validateInvoice(invoice);
  if (validationErrors.length > 0) {
    throw new InvoiceValidationError(validationErrors);
  }
...
const updateInvoice = async (id: string, invoice: Partial<Invoice>) => {
  const validationErrors = validateInvoice({ ...invoice, id });
  if (validationErrors.length > 0) {
    throw new InvoiceValidationError(validationErrors);
  }
...
const markAsPaid = async (id: string, paymentDetails: {
  method: string;
  date: string;
  amount: number;
  reference?: string;
  ...
  try {
    const response = await apiService.put<Invoice>(`/invoices/${id}/mark-paid`, paymentDetails);
    return response;
  } catch (error) {
...
const sendInvoice = async (id: string, emailData: {
  to: string;
  subject?: string;
  message?: string;
}) => {
...
const downloadInvoice = async (id: string, format: 'pdf' | 'csv' = 'pdf') => {
  try {
    const response = await apiService.get(`/invoices/${id}/download?format=${format}`);
    return response;
  } catch (error) {
...
const getInvoicesByCustomer = async (customerId: string, page: number = 1, limit: number = 10) => {
  try {
    const response = await apiService.get<InvoiceResponse>(
...
const getInvoicesByDateRange = async (startDate: string, endDate: string, page: number = 1, limit: number = 10) => {
  try {
    const response = await apiService.get<InvoiceResponse>(
