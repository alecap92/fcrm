...
export function InvoiceHeader({ organization, invoiceNumber, date, dueDate }: InvoiceHeaderProps) {
  const formatAddress = () => {
    if (!organization) return '';
    const addressParts = [];
    if (organization.address) {
      addressParts.push(organization.address);
    }
    if (organization.city) {
      addressParts.push(organization.city);
    }
    if (organization.state) {
      addressParts.push(organization.state);
    }
