...
function ConfirmDialog({ isOpen, post, newDate, onConfirm, onCancel }: ConfirmDialogProps) {
        ...
        <div className="bg-gray-50 rounded-lg p-3 mb-4">
          <div className="flex items-center gap-2 mb-2">
            {post.platforms.map(platform => (
              <span key={platform} className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-gray-100 text-xs">
                {platform === 'instagram' ? (
...
export function Calendar({ posts, onSelectPost, onSelectSlot, onMovePost }: CalendarProps) {
  ...
  const events = useMemo(() => posts.map(post => ({
    ...
    allDay: false,
    isDragging: draggedEvent?.id === post.id,
  })), [posts, draggedEvent]);
    ...
    <div className="flex items-center justify-between p-3 border-b">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className={`px-3 py-1 text-sm rounded-md ${view === Views.MONTH ? 'bg-action text-white' : 'hover:bg-gray-100'}`}
        >
          Mes
        </button>
        <button
          onClick={() => onView(Views.WEEK)}
          className={`px-3 py-1 text-sm rounded-md ${view === Views.WEEK ? 'bg-action text-white' : 'hover:bg-gray-100'}`}
        >
          Semana
        </button>
        <button
          onClick={() => onView(Views.DAY)}
          className={`px-3 py-1 text-sm rounded-md ${view === Views.DAY ? 'bg-action text-white' : 'hover:bg-gray-100'}`}
        >
          Día
        </button>
      </div>
