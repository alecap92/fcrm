...
export function IntegrationsSettings() {
      ...
      try {
        ...
        if (response.data) {
          setIntegrations((prev: any) =>
            prev.map((provider: any) =>
              provider.id === editingProvider?.id
                ? { ...response.data, credentials: editingProvider.credentials }
                : provider
            )
          );
          ...
          setHasChanges(false);
          setEditingProvider(null); // cerrar modal
        }
      } catch (error) {
  ...
  const handleToggleActive = async (providerId: string) => {
    try {
      ...
      if (provider) {
        showLoading(provider.isActive ? "Desactivando integración..." : "Activando integración...");
        ...
        if (response.data) {
          ...
          toast.show({
            title: "Éxito",
            description: `Integración ${provider.isActive ? "desactivada" : "activada"} correctamente`,
            type: "success"
          });
  ...
  const isProviderConfigured = (providerId: string) => {
    return integrations.some(p => p.id === providerId);
  };
    ...
    <div className="p-6">
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-lg border p-8 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">No hay integraciones configuradas</h3>
          <p className="text-sm text-gray-500 max-w-sm mx-auto mb-6">
            Conecta tu aplicación con servicios externos para ampliar su funcionalidad
          </p>
          <Button onClick={() => setShowAddModal(true)}>
            <Plus className="w-4 h-4 mr-2" />
            Agregar Integración
                ...
                >
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <div className={`p-2 rounded-lg ${provider.isActive ? "bg-green-100" : "bg-gray-100"}`}>
                      <Icon className={`w-5 h-5 ${provider.isActive ? "text-green-600" : "text-gray-600"}`} />
                    </div>
                    <div>
                  ...
                  <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <div className="flex-1 flex items-center">
                          ...
                          <Button 
                            variant="ghost" 
                            size="sm"
                            onClick={() => handleCopyToClipboard(provider.webhookUrl || "", `webhook-${provider.id}`)}
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className={`text-gray-600 ${provider.isConfigured ? "text-green-600" : "text-yellow-600"}`}>
                        {provider.isConfigured ? "Configurado" : "No configurado"}
                      </span>
                    </div>
                      ...
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <div className="flex-1 flex items-center">
                          ...
                          <Button 
                            variant="ghost" 
                            size="sm"
                            onClick={() => window.open(provider.apiEndpoint, '_blank')}
                  ...
                  <div className="mt-4 pt-4 border-t">
                    <div className="flex justify-between items-center">
                      <span className={`text-sm ${provider.isActive ? "text-green-600" : "text-gray-500"}`}>
      ...
      {editingProvider && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-medium text-gray-900">
                {editingProvider.isConfigured ? "Editar" : "Configurar"}{" "}
                  ...
                  <div key={field.key}>
                    ...
                    <div className="relative">
                      {field.type === "textarea" ? (
                        ...
                        <div className="flex">
                          ...
                          <button
                            type="button"
                            onClick={() => handleCopyToClipboard(editingProvider.credentials[field.key] || "", `credential-${field.key}`)}
                    ...
                    {field.key === "apiKey" && (
                      <p className="mt-1 text-xs text-gray-500">
                        Puedes encontrar tu API key en el panel de control de {editingProvider.name}.
                      </p>
                    )}
                  </div>
                ))}

              {editingProvider.type === "whatsapp" && (
                <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-100">
                  ...
                  <p className="text-sm text-blue-700">
                    Para completar la configuración de WhatsApp, deberás configurar un webhook en la plataforma de Meta for Developers.
                  </p>
                  <a 
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  ¿Estás seguro de que quieres eliminar la integración con {deleteConfirmation.providerName}? Esta acción no se puede deshacer.
                </p>
              </div>
            </div>
            <div className="flex justify-end gap-3">
              <Button
                variant="outline"
                onClick={() => setDeleteConfirmation({ isOpen: false, providerId: "", providerName: "" })}
              >
                Cancelar
              </Button>
              <Button
                variant="destructive"
                onClick={() => handleDeleteProvider(deleteConfirmation.providerId)}
              >
                Eliminar
              </Button>
            </div>
