...
import type { Pipeline, PipelineColumn, DealCustomField } from '../../types/settings';
...
export function DealsSettings() {
  ...
  const [editingField, setEditingField] = useState<DealCustomField | null>(null);
  ...
  const handleAddColumn = (pipelineId: string) => {
    const newColumn: PipelineColumn = {
      ...
      name: 'New Column',
      color: '#6B7280',
      position: dealsConfig.columns.filter(c => c.pipelineId === pipelineId).length,
      pipelineId,
    };
      ...
      <div className="space-y-6">
                  ...
                  <tr key={pipeline.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        pipeline.isDefault ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                      }`}>
                        <CircleDot className="w-3 h-3 mr-1" />
                        {pipeline.isDefault ? 'Por defecto' : 'No'}
                      </span>
                    </td>
        ...
        <div className="bg-white rounded-lg border p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button onClick={() => handleAddColumn(dealsConfig?.pipelines[0]?.id)}>
              <Plus className="w-4 h-4 mr-2" />
              Nueva Columna
            </Button>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {dealsConfig?.columns.map((column) => (
                  <tr key={column.id}>
                    <td className="pl-4">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-500">
                        {dealsConfig?.pipelines.find(p => p.id === column.pipelineId)?.name}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-1">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          disabled={column.position === dealsConfig.columns.length - 1}
                        >
        ...
        <div className="bg-white rounded-lg border p-4">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-medium text-gray-900">Campos Personalizados</h3>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {dealsConfig?.customFields.map((field) => (
                  <tr key={field.id}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">
                        {field.name}
