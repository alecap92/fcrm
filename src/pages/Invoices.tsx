...
export function Invoices() {
  ...
  const loadInvoices = async () => {
    try {
      setIsLoading(true);
      const response = await invoiceService.getInvoices(currentPage, itemsPerPage);
  ...
  const handleMarkAsPaid = async (id: string) => {
    try {
      await invoiceService.markAsPaid(id, {
        ...
        amount: invoices.find(inv => inv.id === id)?.total || 0
      });
      toast.show({
        title: 'Success',
                ...
                <tr>
                  <th className="px-6 py-3 bg-gray-50 text-left">
                    <input
                      ...
                      checked={selectedInvoices.length === (invoices?.length || 0)}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-col text-sm text-gray-500">
                          ...
                          <span className="text-xs">
                            Vence: {format(new Date(invoice.dueDate), 'dd/MM/yyyy')}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
          ...
          <div className="px-6 py-4 border-t flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-500">
                Mostrando {Math.min((currentPage - 1) * itemsPerPage + 1, totalInvoices)} a{' '}
                {Math.min(currentPage * itemsPerPage, totalInvoices)} de {totalInvoices} facturas
              </span>
            </div>
