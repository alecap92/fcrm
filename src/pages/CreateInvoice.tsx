...
export function CreateInvoice() {
  ...
  const handleUpdateItem = (index: number, field: keyof Invoice['items'][0], value: string | number) => {
    const updatedItems = [...items];
    console.log(index, field, value)
    const item = { ...updatedItems[index], [field]: value };
    console.log(item, 'desde create invoiceeeeeee')
  ...
  const handleSave = async () => {
    try {
      ...
      await invoiceService.createInvoice(invoiceData as Omit<Invoice, "_id" | "createdAt" | "updatedAt">);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <InvoiceNotes
                  notes={invoice.notes}
                  terms={invoice.terms}
                  onNotesChange={(notes) => setInvoice(prev => ({ ...prev, notes }))}
                  onTermsChange={(terms) => setInvoice(prev => ({ ...prev, terms }))}
                />
              </div>
